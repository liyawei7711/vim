...
public class ChatListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    ArrayList<VimMessageListBean> datas;
    private View.OnClickListener ocl;
    private RequestOptions requestFriendHeadOptions;
    private RequestOptions requestGroupHeadOptions;

    public ChatListAdapter(Context context, ArrayList<VimMessageListBean> list, View.OnClickListener ocl) {
        datas = list;
        mContext = context;
        this.ocl = ocl;
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        return new ChatListAdapter.CustomViewHolder(LayoutInflater.from(mContext).inflate(R.layout.item_chat_list_view, parent, false));
    }

    @Override
    public void onBindViewHolder(RecyclerView.ViewHolder holder, final int position) {
        VimMessageListBean bean = datas.get(position);
        final ChatListAdapter.CustomViewHolder viewHolder = (ChatListAdapter.CustomViewHolder) holder;
        viewHolder.message_history_bg.setTag(datas.get(position));
        viewHolder.message_history_bg.setOnClickListener(ocl);
        ...
        if (1 == bean.bFire && (AppUtils.MESSAGE_TYPE_TEXT == bean.type || AppUtils.MESSAGE_TYPE_IMG == bean.type || AppUtils.MESSAGE_TYPE_AUDIO_FILE == bean.type || AppUtils.MESSAGE_TYPE_VIDEO_FILE == bean.type)) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.yuehoujifen) + "]");
        } else if (bean.type == MESSAGE_TYPE_IMG) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.img) + "]");
//            left_Image.setImageResource("0".equals(bean.sessionID)?R.drawable.zhilingdiaodu_guangbo:R.drawable.zhilingdiaodu_putongxiaoxi);
            ...
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.notice_file) + "]");
//            left_Image.setImageResource("0".equals(bean.sessionID)?R.drawable.zhilingdiaodu_guangbo:R.drawable.zhilingdiaodu_putongxiaoxi);
        } else if (bean.type == MESSAGE_TYPE_TEXT) {
            showTextContent2(bean, viewHolder.item_content);
//            left_Image.setImageResource("0".equals(bean.sessionID)?R.drawable.zhilingdiaodu_guangbo:R.drawable.zhilingdiaodu_putongxiaoxi);
            ...
            if (bean.type == NOTIFICATION_TYPE_PERSON_PUSH) {
                viewHolder.item_content.setText("[" + AppUtils.getString(R.string.person_share) + "]");
            } else {
                viewHolder.item_content.setText("[" + AppUtils.getString(R.string.device_share) + "]");
            }
//            left_Image.setImageResource("0".equals(bean.sessionID)?R.drawable.zhilingdiaodu_guangbo:R.drawable.zhilingdiaodu_tuisong);
            ...
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.audio) + "]");
//            left_Image.setImageResource(R.drawable.zhilingdiaodu_guangbo);
        } else if (bean.type == MESSAGE_TYPE_VIDEO_FILE) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.video) + "]");
//            left_Image.setImageResource(R.drawable.zhilingdiaodu_guangbo);
        } else if (bean.type == MESSAGE_TYPE_SINGLE_CHAT_VOICE) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.chat_voice_content) + "]");
//            left_Image.setImageResource(R.drawable.zhilingdiaodu_guangbo);
        } else if (bean.type == MESSAGE_TYPE_SINGLE_CHAT_VIDEO) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.chat_video_content) + "]");
//            left_Image.setImageResource(R.drawable.zhilingdiaodu_guangbo);
        } else if (bean.type == MESSAGE_TYPE_ADDRESS) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.chat_address) + "]");
        } else if (bean.type == MESSAGE_TYPE_GROUP_MEET) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.chat_group_video) + "]");
        } else if (bean.type == MESSAGE_TYPE_JINJI) {
            viewHolder.item_content.setText("[" + AppUtils.getString(R.string.chat_jinji) + "]");
        } else if (bean.type == MESSAGE_TYPE_SHARE) {
            showTextContent2(bean, viewHolder.item_content);
        } else if (bean.type == CHAT_CONTENT_CUSTOM_NOTICE_ITEM) {
            viewHolder.item_content.setText(bean.msgTxt);
        } else {
...
//        }
        if (bean.groupType == 1 || bean.groupType == 2) {
            ...
            viewHolder.view_point.setText(num > 99 ? "99+" : num + "");
        } else {
            int num = AppDatas.MsgDB()
                    .chatSingleMsgDao()
        ...
        if (bean.nMsgTop == 1) {
            viewHolder.message_history_bg.setBackgroundResource(R.color.back_feeeee);
        } else {
            viewHolder.message_history_bg.setBackgroundResource(R.color.white);
        }

        if (bean.nNoDisturb == 1) {
            viewHolder.message_history_miandarao.setVisibility(View.VISIBLE);
        } else {
            viewHolder.message_history_miandarao.setVisibility(View.GONE);
        }
        setHeadImage(viewHolder.left_Image, bean);
    ...
    private class CustomViewHolder extends RecyclerView.ViewHolder {
        ...
        CustomViewHolder(View itemView) {
            super(itemView);
            view_point = (TextView) itemView.findViewById(R.id.view_point);
            ...
            message_history_miandarao = (ImageView) itemView.findViewById(R.id.message_history_miandarao);
            message_history_bg = (RelativeLayout) itemView.findViewById(R.id.message_history_bg);
